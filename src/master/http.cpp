...
struct FrameworkWriter {
  ...
  void operator()(JSON::ObjectWriter* writer) const {
    json(writer, Summary<Framework>(*fwk));

    // Add additional fields to those generated by the
    // `Summary<Framework>` overload.
    writer->field("user", fwk->info.user());
    writer->field("failover_timeout", fwk->info.failover_timeout());
    writer->field("checkpoint", fwk->info.checkpoint());
    writer->field("role", fwk->info.role());
    writer->field("registered_time", fwk->registeredTime.secs());
    writer->field("unregistered_time", fwk->unregisteredTime.secs());

    if (fwk->info.has_principal()) {
      writer->field("principal", fwk->info.principal());
    }
    ...
    writer->field(
        "resources",
        fwk->totalUsedResources + fwk->totalOfferedResources);

    // TODO(benh): Consider making reregisteredTime an Option.
    if (fwk->registeredTime != fwk->reregisteredTime) {
      writer->field("reregistered_time", fwk->reregisteredTime.secs());
    }

    // Model all of the tasks associated with a framework.
    writer->field("tasks", [this](JSON::ArrayWriter* writer) {
      foreachvalue (const TaskInfo& taskInfo, fwk->pendingTasks) {
        // Skip unauthorized tasks.
        if (!filter->filter(taskInfo)) {
          continue;
        }

        writer->element([this, &taskInfo](
            JSON::ObjectWriter* writer) {
          writer->field("id", taskInfo.task_id().value());
          writer->field("name", taskInfo.name());
          writer->field("framework_id", fwk->id().value());
      ...
      foreachvalue (Task* task, fwk->tasks) {
        // Skip unauthorized tasks.
    ...
    writer->field("completed_tasks", [this](
        JSON::ArrayWriter* writer) {
      foreach (const std::shared_ptr<Task>& task, fwk->completedTasks) {
        // Skip unauthorized tasks.
        if (!filter->filter(*task)) {
          continue;
        }
    ...
    writer->field("offers", [this](JSON::ArrayWriter* writer) {
      foreach (Offer* offer, fwk->offers) {
    ...
    if (fwk->info.has_labels()) {
      writer->field("labels", fwk->info.labels());
    }
  }
    ...
    const Option<string>& /*principal*/) const
{
  ...
  collect(filterList).onAny(
    ...
    auto state = [this, &filterLists](JSON::ObjectWriter* writer) {
      ...
      writer->field("frameworks", [this, &tasksFilter, &frameworksFilter](
        ...
        foreachvalue (Framework* framework, master->frameworks.registered) {
          ...
          auto fwkWriter = FrameworkWriter(tasksFilter, framework);
      ...
      // Model all of the completed frameworks.
      writer->field("completed_frameworks", [this, &tasksFilter](
           JSON::ArrayWriter* writer) {
        foreach (const std::shared_ptr<Framework>& framework,
                 master->frameworks.completed) {
          auto fwkWriter = FrameworkWriter(tasksFilter, framework.get());
      ...
      writer->field("orphan_tasks", [this,  &tasksFilter](
          JSON::ArrayWriter* writer) {
