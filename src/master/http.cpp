...
struct FrameworkWriter {
  ...
  void operator()(JSON::ObjectWriter* writer) const {
    ...
    writer->field("completed_tasks", [this, &framework](
    ...
    writer->field("executors", [this, &framework](JSON::ArrayWriter* writer) {
      foreachpair (
...
Future<bool> Master::Http::authorizeExecutorInfo(
    const Option<string>& principal,
    const ExecutorInfo& executorInfo) const
{
  ...
  string object;
  if (!executorInfo.SerializeToString(&object)) {
    // Error.
    return false;
  }
  request.mutable_object()->set_value(object);
...
Future<bool> Master::Http::authorizeTask(
    const Option<string>& principal,
    const Task& task) const
{
  authorization::Request request;
  request.set_action(authorization::VIEW_TASK_WITH_TASK);
...
// Creates authorized view of master datastructures.
Future<Master::Http::AuthorizedMasterView> Master::Http::createMasterViewState(
    const Master* master,
    const Option<string>& principal,
    const bool httpAuthorizationFiltering) const
{
  std::shared_ptr<process::Promise<hashset<FrameworkID>>> frameworksPromise(
      new process::Promise<hashset<FrameworkID>>);

  std::shared_ptr<process::Promise<hashset<ExecutorID>>> executorsPromise(
      new process::Promise<hashset<ExecutorID>>);

  std::shared_ptr<process::Promise<hashset<TaskID>>> tasksPromise(
      new process::Promise<hashset<TaskID>>);
  ...
  if (httpAuthorizationFiltering) {
    ...
    list<Future<bool>> authorizedExecutors;
    list<TaskID> taskIds;
    ...
    collect(authorizedEntities).onAny([=](
      ...
      if (authorizedEntities.isReady()) {
        AuthorizedMasterView masterView;
        masterView.httpAuthorizationFiltering = true;

        CHECK_EQ(authorizedEntities.get().size(), 3u);
        auto entityIterator = authorizedEntities.get().begin();
        list<bool> authorizedFrameworks = *entityIterator;
        ++entityIterator;
        list<bool> authorizedExecutors = *entityIterator;
        ++entityIterator;
        list<bool> authorizedTasks = *entityIterator;

        CHECK_EQ(authorizedFrameworks.size(), frameworkIds.size());
        CHECK_EQ(authorizedExecutors.size(), executorIds.size());
        CHECK_EQ(authorizedTasks.size(), taskIds.size());

        // Check authorized frameworks.
        // We manually loop over both lists synchronously.
        auto frameworksIterator = frameworkIds.begin();
        auto boolIterator = authorizedFrameworks.begin();
        for (; frameworksIterator != frameworkIds.end() &&
            boolIterator != authorizedFrameworks.end();
            ++frameworksIterator, ++boolIterator) {
          if (*boolIterator) {
            // Collect authorized frameworkIds.
            masterView.authorizedFrameworkIds.insert(*frameworksIterator);
          }
        }
        ...
        // We manually loop over both lists synchronously.
        auto executorIterator = executorIds.begin();
        boolIterator = authorizedExecutors.begin();
        for (; executorIterator != executorIds.end() &&
            boolIterator != authorizedExecutors.end();
            ++executorIterator, ++boolIterator) {
          if (*boolIterator) {
            // Collect authorized executorsIds.
            masterView.authorizedExecutorIds.insert(*executorIterator);
          }
        }

        // Check authorized tasks.
        // We manually loop over both lists synchronously.
        auto taskIterator = taskIds.begin();
        boolIterator = authorizedTasks.begin();
        for (; taskIterator != taskIds.end() &&
            boolIterator != authorizedTasks.end();
            ++taskIterator, ++boolIterator) {
          if (*boolIterator) {
            // Collect authorized frameworkIds.
            masterView.authorizedTaskIds.insert(*taskIterator);
          }
        }

        promise->set(masterView);
      } else {
        if (authorizedEntities.isFailed()) {
          promise->fail("Authorization failed: Future failed");
        }
        else {
          promise->fail(
              "Authorization failed: Future discarded");
        }
      }
    });
  } else {
