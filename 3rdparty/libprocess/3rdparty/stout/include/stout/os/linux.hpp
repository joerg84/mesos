...
inline pid_t clone(const lambda::function<int()>& func, int flags)
{
  ...
  // TODO(joerg84): Investigate whether cleanup is needed in case of CLONE_VM.
  if (!(flags & CLONE_VM)) {
    delete[] stack;
  }
...
// Direct clone without the use of a helper function.
inline pid_t clone_d( int(func)(void*), void* args, int flags)
{
  // Stack for the child.
  // - unsigned long long used for best alignment.
  // - 8 MiB appears to be the default for "ulimit -s" on OSX and Linux.
